...
import Distribution.Dev.Flags (Config, GlobalFlag (Version), fromFlags, getEnvVars, getOpt'', getSandbox, getVerbosity, globalOpts, helpRequested,
                               parseGlobalFlags)
...
main = do
  ...
  case args of
    (name:args') -> case nameCmd name of
      Just cmdAct
        | helpRequested globalFlags -> do
            putStrLn $ cmdDesc cmdAct
            putStr =<< globalUsage
            exitWith ExitSuccess
        | otherwise -> runCmd cmdAct cfg args'
      Nothing -> do
        putStrLn $ "Unknown command: " ++ show name
        putStr =<< globalUsage
        exitWith (ExitFailure 1)
...
wrap w orig = snd $ evalState (go 0 orig) Map.empty
  where
    ...
    scoreSplit loc (offset, lineScore, line, s') = case s' of
...
splits w s = map (\k -> score k $ splitAt k s) [w - 1,w - 2..1]
    where
      ...
      score k (r, [])       = (k, 0, r, [])
      score k (r, cs@(c:_)) =
        let (sps, cs') = countDropSpaces 0 cs
