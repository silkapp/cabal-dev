...
-}
module Distribution.Dev.AddSource (actions) where
...
import Distribution.PackageDescription (BuildType (Custom), PackageDescription (buildType), package, packageDescription)
import Distribution.PackageDescription.Parse (ParseResult (ParseOk), parsePackageDescription, readPackageDescription)
import Distribution.Text (display, simpleParse)
import Network.HTTP (RequestMethod (GET), mkRequest, rspBody, simpleHTTP)
import Network.URI (URI, parseURI, uriPath, uriScheme)
import System.Console.GetOpt (OptDescr (..))
import System.Directory (copyFile, createDirectoryIfMissing, getCurrentDirectory, getDirectoryContents, getTemporaryDirectory, removeFile, renameFile,
                         setCurrentDirectory)
import System.Exit (ExitCode (..))
import System.FilePath (splitDirectories, splitExtension, takeBaseName, takeExtension, (<.>), (</>))
import System.IO (IOMode (..), hClose, hFlush, hSetBinaryMode, openTempFile, withBinaryFile)
import System.IO.Error (isDoesNotExistError)
import System.Process (rawSystem)
import qualified Control.Exception as Ex (bracket, catch)

import qualified Codec.Archive.Tar          as T (Entries, Entry, EntryContent (NormalFile), FormatError, entryContent, entryPath, foldEntries, read, write)
import qualified Codec.Archive.Tar.Entry    as T (entryTarPath, fileEntry, toTarPath)
import qualified Codec.Compression.GZip     as Z (decompress)
import qualified Data.ByteString.Lazy       as L (ByteString, hGetContents, hPut, length)
import qualified Data.ByteString.Lazy.Char8 as L8 (unpack)
import qualified Distribution.Verbosity     as V (Verbosity, normal)

import Distribution.Dev.Command (CommandActions (..), CommandResult (..))
import Distribution.Dev.Flags (Config, getVerbosity)
import Distribution.Dev.Sandbox (Sandbox, indexCache, indexTar, indexTarBase, localRepoPath, resolveSandbox)
...
addSources flgs fns = do
  ...
  debug v $ "Making a cabal repo in " ++ localRepoPath sandbox ++ " out of " ++ show fns
  results <- mapM (processLocalSource v) fns
  let errs = [e | Left e <- results]
  let srcs = [s | Right s <- results]
  if not $ null errs
    then return $ CommandError $ unlines $ "Errors finding cabal files:":map (showString "  ") errs
    else do
      let (sources, newEntries) = unzip srcs
      res <- readExistingIndex sandbox
      case res of
        Left err -> return $ CommandError $ "Error reading existing index: " ++ err
        Right existingIndex -> do
...
writeIndex :: Sandbox a -- ^The local repository path
           ...
           -> IO ()
writeIndex sandbox ents = do
  newIndexName <- withTmpIndex $ \(fn, h) -> do
  ...
  where
    withTmpIndex = Ex.bracket (openTempFile (localRepoPath sandbox) indexTarBase) (hClose . snd)
...
toIndexEntry pkgId c = right (`T.fileEntry` c) $ T.toTarPath False (indexName pkgId)
...
readExistingIndex sandbox =
  ...
  where
    readIndexFile = withBinaryFile (indexTar sandbox) ReadMode $ forceEntries . T.read <=< L.hGetContents
    forceEntries es = either (const 0) length es' `seq` return es'
      where
        step _ l@(Left _) = l
...
installTarball :: Config
               ...
               -> IO (Either String ())
installTarball flgs sandbox src pkgId pkgDesc = do
  createDirectoryIfMissing True $ localRepoPath sandbox </> repoDir pkgId
  case src of
    TarPkg fn -> do
      copyFile fn dest
      return $ Right ()
      ...
      Ex.bracket getCurrentDirectory setCurrentDirectory $ \_ -> do
        ...
        case cabalRes of
          ExitSuccess -> do
            ...
            return $ Right $ here </> "dist" </> display pkgId <.> "tar" <.> "gz"
          ExitFailure code -> return $ Left $ "cabal sdist failed with " ++ show code

downloadTarball :: URI -> IO (Either String FilePath)
downloadTarball u = do
  tmpLoc <- getTemporaryDirectory
  Ex.bracket (openTempFile tmpLoc "package-.tar.gz") (hClose . snd) $ \(fn, h) -> do
    httpRes <- simpleHTTP $ mkRequest GET u
    case httpRes of
      Left err -> return . Left $ show err
      Right resp -> do
        L.hPut h $ rspBody resp
        return $ Right fn
...
-- file
processDirectory :: V.Verbosity -> FilePath -> IO (Either String (PackageIdentifier, L.ByteString, PackageDescription))
...
extractCabalFile = T.foldEntries step Nothing (const Nothing)
  where
    ...
    entPackageId ent = case splitDirectories $ T.entryPath ent of
      [d, f] -> do
        i <- simpleParse d
...
repoDir pkgId = display (pkgName pkgId) </> display (pkgVersion pkgId)
